...
"""
    ...
    def call_gemini(self, task_prompt, context={}):
        """Main AI call — named call_gemini for compatibility, uses Groq"""
        max_retries = 3
            ...
            try:
                ...
                if "429" in error or "rate" in error.lower():
                    ...
                    self.log_and_broadcast(
                        f"⏳ [{self.name}] Rate limit hit — waiting {wait}s — attempt {attempt+1}/{max_retries}",
                        "WARNING"
                    )
                    time.sleep(wait)
                else:
                    self.log_and_broadcast(f"❌ [{self.name}] Error: {error[:200]}", "ERROR")
    ...
    def broadcast(self, content, log_type="INFO"):
        try:
            ...
            broadcast_log(self.name, log_type, str(content)[:2000])
        except Exception as e:
            pass

    def log_and_broadcast(self, content, log_type="INFO"):
        print(f"[{datetime.now().strftime('%H:%M:%S')}][{self.name}][{log_type}] {str(content)[:200]}")
    ...
    def get_performance_context(self):
        try:
            ...
            with app.app_context():
                ...
                analytics = Analytics.query.order_by(Analytics.date.desc()).limit(7).all()
